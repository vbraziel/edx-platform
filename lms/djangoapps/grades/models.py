...
class PersistentSubsectionGradeOverride(models.Model):
    ...
    @classmethod
    def update_or_create_override(cls, requesting_user, subsection_grade_model, **override_data):
        """
    ...
    def _prepare_override_params(subsection_grade_model, override_data):
        ...
        for field in override_data.keys():
            if field in allowed_fields_and_defaults:
                cleaned_data[field] = override_data[field]

        for override_field, default_field in allowed_fields_and_defaults.items():
            if override_field not in cleaned_data:
                cleaned_data[override_field] = getattr(subsection_grade_model, default_field)
